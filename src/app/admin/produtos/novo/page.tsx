...
export default function NovoProdutoPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/categorias").then((r) => r.json()).then((d) => {
  ...
  const label: React.CSSProperties = { fontSize: 12, color: "#9e9a90", display: "block", marginBottom: 6, fontWeight: 500 };
    ...
    <AdminLayout>
      <div style={{ maxWidth: 700 }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#f0ede6", margin: 0 }}>Novo Produto</h1>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ background: "#111420", borderRadius: 16, padding: 24, border: "1px solid rgba(255,255,255,0.07)", marginBottom: 16 }}>
            ...
            {imagePreview || form.imageUrl ? (
              <div style={{ marginBottom: 12, position: "relative", display: "inline-block" }}>
                ...
                <img
                  ...
                  alt="Preview"
                  style={{ width: 120, height: 120, borderRadius: 10, objectFit: "cover", display: "block" }}
                  onError={(e) => { (e.target as HTMLImageElement).style.opacity = "0.3"; }}
                />
                <button
                  type="button"
                  onClick={() => { set("imageUrl", ""); setImagePreview(""); }}
                  style={{
                    position: "absolute", top: -8, right: -8, width: 24, height: 24,
                    borderRadius: "50%", background: "#e84040", border: "none",
            ...
            <div
              ...
              style={{
                border: "2px dashed rgba(255,255,255,0.1)", borderRadius: 12, padding: "28px 20px",
                textAlign: "center", cursor: "pointer", transition: "border-color 0.15s",
                background: "rgba(255,255,255,0.02)",
              }}
            >
              <div style={{ fontSize: 32, marginBottom: 8 }}>📷</div>
              <div style={{ fontSize: 13, color: "#9e9a90", marginBottom: 4 }}>
                {uploading ? "Enviando..." : "Arraste uma imagem ou clique para selecionar"}
              </div>
              <div style={{ fontSize: 11, color: "#5a5650" }}>JPG, PNG, WEBP · Máximo 5MB</div>
              <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" style={{ display: "none" }}
                onChange={(e) => { const f = e.target.files?.[0]; if (f) handleImageUpload(f); }} />
            </div>

            <div style={{ marginTop: 12 }}>
              <label style={label}>Ou cole uma URL de imagem</label>
              <input
                type="text"
                value={form.imageUrl}
          ...
          <div style={{ background: "#111420", borderRadius: 16, padding: 24, border: "1px solid rgba(255,255,255,0.07)", marginBottom: 20 }}>
            ...
            {[
              ...
              <div key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                <div>
                  <div style={{ fontSize: 13, color: "#f0ede6", fontWeight: 500 }}>{lbl}</div>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: form[key as keyof typeof form] ? "#c9a84c" : "rgba(255,255,255,0.1)",
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <button
              ...
              disabled={saving}
              style={{
                flex: 1, padding: "13px 0", background: saving ? "#5a5650" : "linear-gradient(135deg,#c9a84c,#e4c97e)",
                border: "none", borderRadius: 10, cursor: saving ? "not-allowed" : "pointer",
                color: "#0b0d12", fontWeight: 700, fontSize: 14, fontFamily: "DM Sans, sans-serif",
              }}
            >
            ...
            <button
              ...
              onClick={() => router.back()}
              style={{
                padding: "13px 20px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)",
                borderRadius: 10, cursor: "pointer", color: "#f0ede6", fontSize: 14,
